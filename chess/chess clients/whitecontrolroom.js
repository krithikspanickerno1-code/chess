/* chesscontrolroom_white.js
   White client logic (separate file)
   - Keeps UI and game logic for White client
   - Polls server for state and posts moves
   - Only allows White to move when server indicates whiteTurn === true
   - Local fallback if server unreachable
   - No en-passant visual dot
   - Promotion UI included
*/
...
canvas.addEventListener("click", (e) => {
  if(gameOver) return;
  if(!whiteTurn){ // this client only moves when whiteTurn is true
  ...
  if(!selected){
    ...
    } else if(piece && isWhitePiece === true){
      selected = { row, col };
      legalMoves = getLegalMoves(board, row, col, enPassantTarget, castlingRights);
    } else {
      selected = null; legalMoves = [];
...
async function fetchStateFromServer(){
  try {
    ...
    gameOver = !!s.gameOver;
    // positionLog not synced from server in this simple model
    render();
  } catch (err) {
...
document.getElementById("offer-draw").addEventListener("click", async () => {
  ...
  if (accept) {
    ...
    try {
      await fetch(`${SERVER_BASE}/draw`, { method: "POST" });
    } catch(e){}
  } else {
...
document.getElementById("resign").addEventListener("click", async () => {
  ...
  try {
    await fetch(`${SERVER_BASE}/resign`, { method: "POST", headers: { "Content-Type":"application/json" }, body: JSON.stringify({ side: "white" }) });
  } catch(e){}
});
...
document.getElementById("reset").addEventListener("click", async () => {
  resetGame();
  try {
    await fetch(`${SERVER_BASE}/reset`, { method: "POST" });
  } catch(e){}
});
